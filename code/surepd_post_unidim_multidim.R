...
md = stan_model('source_code/surepd_multidim_loglik.stan')
...
save(list = 'fitStan', file = 'RData/SurePD_post_multidim.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+2*2+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_post_multidim.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_post_multidim.csv')
...
save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_post_multidim_beta.RData')

beta_f1 <- beta_f1_sample$beta_f1
beta_f2 <- beta_f2_sample$beta_f2

kk <- beta_f1[, 1] + beta_f1[, 2]
mean(kk)
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta_f1 = beta_f1[1]*time[i] + beta_f1[2]*time[i]*trt[i] + U[id[i], 1]
...
## Compute DIC
np <- sum(items.ncat) + 2*2 + 1 ## number of parameters
log_predictive_probability <- sum(ll_posterior) + (np/2)*log(2*pi)
Dhat <- -2*sum(ll_posterior)

DIC <- 2*Dbar - Dhat

save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_post_multidim.RData')



#### Unidimensional analysis ####
md = stan_model('source_code/surepd_unidim_loglik.stan')
stan_dat <- list(n = N, nobs = nrow(long.data),
                 items = n.items,
                 id = long.data$ID,
                 time = long.data$VisitYr, trt = long.data$TRT_ACTV,
...
fitStan <- sampling(md, data = stan_dat, iter = 4000, warmup = 3000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_post_unidim.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+2), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_post_unidim.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_post_unidim.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_post_unidim_beta.RData')
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta = beta[1]*time[i] + beta[2]*time[i]*trt[i] + U[id[i]]
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_post_unidim.RData')
