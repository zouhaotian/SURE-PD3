...
## Time spline matrix: non-tremor domain only ##
time <- long.data$VisitYr
trt <- long.data$TRT_ACTV
tau <- c(0, 1) ## based on LOWESS plot
n.tau <- length(tau)

## corresponds to (t_i - tau_k)+
spline.matrix <- matrix(NA, nrow(long.data), n.tau*2) 
for (i in 1:length(time)){
  tmp.time.spline <- ifelse(time[i]>tau, time[i]-tau, 0)
  tmp.trt.spline <- trt[i]*tmp.time.spline
  spline.matrix[i, ] <- c(tmp.time.spline, tmp.trt.spline)
}
P <- ncol(spline.matrix) ## number of regression coefficients in theta
...
md = stan_model('source_code/surepd_multidim_spline_loglik.stan')
stan_dat <- list(n = N, nobs = nrow(long.data), P = P,
                 items_f1 = n.items.f1, items_f2 = n.items.f2, items = n.items,
                 id = long.data$ID,
                 time_spline = spline.matrix,
...
pars <- c('a_temp', 'delta','location', 'b', 'beta_f1', 'beta_f2', 'rho','U', 'll')
fitStan <- sampling(md, data = stan_dat, iter = 4000, warmup = 3000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_multidim_spline.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P+2+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_multidim_spline.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_multidim_spline.csv')
...
rho_sample <- extract(fitStan, 'rho')
U_sample <- extract(fitStan, 'U')
ll_sample <- extract(fitStan, 'll')

save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_multidim_spline_beta.RData')
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta_f1 = sum(beta_f1*spline.matrix[i, ]) + U[id[i], 1]
  theta_f2 = beta_f2[1]*time[i] + beta_f2[2]*time[i]*trt[i] + U[id[i], 2]
...
## Compute DIC
np <- sum(items.ncat) + P+2 + 1 ## number of parameters
log_predictive_probability <- sum(ll_posterior) + (np/2)*log(2*pi)
Dhat <- -2*sum(ll_posterior)

DIC <- 2*Dbar - Dhat

save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_multidim_spline.RData')



#### Unidimensional analysis ####
md = stan_model('source_code/surepd_unidim_spline_loglik.stan')
stan_dat <- list(n = N, nobs = nrow(long.data), P = P,
                 items = n.items,
                 id = long.data$ID,
                 time_spline = spline.matrix,
...
inits2 <- list(a_temp = rep(-1, n.items), 
               ...
               beta = rep(0.3, P))
inits <- list(c1 = inits1, c2 = inits2)
pars <- c('a_temp', 'delta','location', 'b', 'beta', 'U', 'll')
fitStan <- sampling(md, data = stan_dat, iter = 3000, warmup = 2000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_unidim_spline.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_unidim_spline.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_unidim_spline.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_unidim_spline_beta.RData')
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta = sum(beta*spline.matrix[i, ]) + U[id[i]]
...
## Compute DIC
np <- sum(items.ncat) + 2 ## number of parameters
log_predictive_probability <- sum(ll_posterior) + (np/2)*log(2*pi)
Dhat <- -2*sum(ll_posterior)

DIC <- 2*Dbar - Dhat

save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_unidim_spline.RData')
