...
save(list = 'fitStan', file = 'RData/SurePD_post_multidim_slope.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+2*2+2+6+4*4), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_post_multidim_slope.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_post_multidim_slope.csv')
...
save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_post_multidim_slope_beta.RData')
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_post_multidim_slope.RData')
...
fitStan <- sampling(md, data = stan_dat, iter = 3000, warmup = 2000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_post_unidim_slope.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+2+1+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_post_unidim_slope.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_post_unidim_slope.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_post_unidim_slope_beta.RData')
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_post_unidim_slope.RData')
