...
md = stan_model('source_code/surepd_multidim_slope_cubic_loglik.stan')
...
save(list = 'fitStan', file = 'RData/SurePD_multidim_slope_cubic.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P+P+2+6+4*4), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_multidim_slope_cubic.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_multidim_slope_cubic.csv')
...
save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_multidim_slope_cubic_beta.RData')
...
for (i in 1:nobs){
  ...
  theta_f2 = sum(beta_f2*spline.matrix[i, ]) + U[id[i], 3] + U[id[i], 4]*time[i]
...
## Compute DIC
np <- sum(items.ncat) + P+P + 2+6 ## number of parameters
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_multidim_slope_cubic.RData')
...
fitStan <- sampling(md, data = stan_dat, iter = 4000, warmup = 3000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_unidim_slope_cubic.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P+1+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_unidim_slope_cubic.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_unidim_slope_cubic.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_unidim_slope_cubic_beta.RData')
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_unidim_slope_cubic.RData')
