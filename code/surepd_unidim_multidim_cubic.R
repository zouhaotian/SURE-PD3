...
spline.matrix <- matrix(NA, nrow(long.data), 3*2) 
for (i in 1:length(time)){
  tmp.time.spline <- c(time[i], time[i]^2, time[i]^3)
...
md = stan_model('source_code/surepd_multidim_spline_general_loglik.stan')
...
save(list = 'fitStan', file = 'RData/SurePD_multidim_cubic.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P*2+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_multidim_cubic.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_multidim_cubic.csv')
...
save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_multidim_cubic_beta.RData')
...
for (i in 1:nobs){
  ...
  theta_f2 = sum(beta_f2*spline.matrix[i, ]) + U[id[i], 2]
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_multidim_cubic.RData')
...
fitStan <- sampling(md, data = stan_dat, iter = 3000, warmup = 2000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_unidim_cubic.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_unidim_cubic.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_unidim_cubic.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_unidim_cubic_beta.RData')
...
## Compute DIC
np <- sum(items.ncat) + P ## number of parameters
log_predictive_probability <- sum(ll_posterior) + (np/2)*log(2*pi)
Dhat <- -2*sum(ll_posterior)

DIC <- 2*Dbar - Dhat

save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), file = 'RData/DIC_SurePD_unidim_cubic.RData')
