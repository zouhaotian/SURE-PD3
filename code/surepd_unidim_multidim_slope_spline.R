...
md = stan_model('source_code/surepd_multidim_slope_spline_loglik.stan')
...
save(list = 'fitStan', file = 'RData/SurePD_multidim_slope_spline.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P+2+2+6+4*4), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_multidim_slope_spline.csv')
...
write.csv(discri_diff_est, 'result/diff_discri_SurePD_multidim_slope_spline.csv')
...
save(list = c('beta_f1_sample', 'beta_f2_sample'), file = 'RData/SurePD_multidim_slope_spline_beta.RData')
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta_f1 = sum(beta_f1*spline.matrix[i, ]) + U[id[i], 1] + U[id[i], 2]*time[i]
...
## Compute DIC
np <- sum(items.ncat) + P+2 + 2+6 ## number of parameters
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_multidim_slope_spline.RData')



#### Unidimensional analysis ####
md = stan_model('source_code/surepd_unidim_slope_spline_loglik.stan')
stan_dat <- list(n = N, nobs = nrow(long.data), P = P,
                 items = n.items,
                 id = long.data$ID,
                 time_spline = spline.matrix,
...
fitStan <- sampling(md, data = stan_dat, iter = 3000, warmup = 2000, 
                    chains = 2, thin=1, init = inits, pars = pars, seed = 123,
                    control = list(adapt_delta = 0.8, max_treedepth=10))
save(list = 'fitStan', file = 'RData/SurePD_unidim_slope_spline.RData')
summ <- summary(fitStan)$summary[1:(sum(items.ncat)*2+P+1+1), c(1, 3, 4, 8, 9, 10)]
write.csv(summ, file = 'result/SurePD_unidim_slope_spline.csv')
...
# rownames(discri_diff_est) <- colnames(items.rename)
colnames(discri_diff_est) <- c('Discrim', '0 to 1', '1 to 2', '2 to 3', '3 to 4')
write.csv(discri_diff_est, 'result/diff_discri_SurePD_unidim_slope_spline.csv')
...
save(list = c('beta_sample'), file = 'RData/SurePD_unidim_slope_spline_beta.RData')
...
for (i in 1:nobs){
  ...
  which_cat <- 0
  which_location <- 0
  theta = sum(beta*spline.matrix[i, ]) + U[id[i], 1] + U[id[i], 2]*time[i]
...
## Compute DIC
np <- sum(items.ncat) + P +1+1 ## number of parameters
log_predictive_probability <- sum(ll_posterior) + (np/2)*log(2*pi)
Dhat <- -2*sum(ll_posterior)
...
save(list = c('DIC', 'Dbar', 'Dhat', 'np', 'log_predictive_probability'), 
     file = 'RData/DIC_SurePD_unidim_slope_spline.RData')
