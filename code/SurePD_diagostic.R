...
save(list = c('y.sim.list', 'proportion.sim.list'), file = 'RData/SurePD_sim_final_check_y_proportion.RData')
...
for (j in 1:length(item.sub)){
  ...
  fname <- paste0('plot/SurePD_mirror_plot_', j, '.png')
...
ggsave(filename = 'plot/SurePD_ICC_plot.png', plot = p2, 
       width = 8, height = 8)
...
ggsave(filename = 'plot/SurePD_Residual_corr_plot.png', plot = p3, 
       width = 8, height = 8)
...
cairo_ps(filename='plot/SurePD_VPC_plot.eps', height=10, width = 6)
