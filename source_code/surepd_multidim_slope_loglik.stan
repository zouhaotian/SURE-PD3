...
parameters{
  ...
  real<lower=-1, upper=1> rho_23; // correlation coefficients
  real<lower=-1, upper=1> rho_02; // correlation coefficients
  real<lower=-1, upper=1> rho_03; // correlation coefficients
  real<lower=-1, upper=1> rho_12; // correlation coefficients
  real<lower=-1, upper=1> rho_13; // correlation coefficients
  vector[4] U[n]; // random intercept and slope for non-tremor and tremor items
}
transformed parameters{
  ...
  for (i in 1:nobs){
    ...
    which_delta = 0;
    which_location = 0;
    theta_f1[i] = beta_f1[1]*time[i] + beta_f1[2]*time[i]*trt[i] + U[id[i], 1] + U[id[i], 2]*time[i];
    theta_f2[i] = beta_f2[1]*time[i] + beta_f2[2]*time[i]*trt[i] + U[id[i], 3] + U[id[i], 4]*time[i];
  ...
  Sigma_u[3, 3] = 1;
  Sigma_u[3, 4] = rho_23*sigma_u2;
  Sigma_u[4, 3] = Sigma_u[3, 4];
  Sigma_u[4, 4] = sigma_u2*sigma_u2;
  
  Sigma_u[1, 3] = rho_02;
  Sigma_u[3, 1] = Sigma_u[1, 3];
  Sigma_u[1, 4] = rho_03*sigma_u2;
  Sigma_u[4, 1] = Sigma_u[1, 4];
  Sigma_u[2, 3] = rho_12*sigma_u1;
  Sigma_u[3, 2] = Sigma_u[2, 3];
  Sigma_u[2, 4] = rho_13*sigma_u1*sigma_u2;
  Sigma_u[4, 2] = Sigma_u[2, 4];
}
model{
  for (i in 1:nobs){
